...
function Game() {
  ...
  useEffect(() => {
    if (document.querySelector(".words")) {
      document.querySelector(".words").addEventListener("keyup", (e) => {
        const key = e.key;
        ...
        if (key != "Backspace") {
          if (currentLetter) {
            cursor.style.top =
              currentLetter.getBoundingClientRect().top + 4 + "px";
            cursor.style.left =
  ...
  return (
    <div className="game">
      <div className="words" tabIndex={0}>
        {gameWords &&
          gameWords.map((word) => (
            <div className="word">
              {[...word].map((c) => (
